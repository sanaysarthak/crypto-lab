# Program for implementing Decryption of DES Algorithm in Python
...
def unpad_text(text):
    # Get the last character (padding value)
    ...
    if pad_value > 0 and pad_value <= 8:
        ...
        for i in range(1, pad_value + 1):
            if ord(text[-i]) != pad_value:
                # Invalid padding, return the text as is
                return text
        
        # Remove padding
        return text[:-pad_value]
    
    # No padding or invalid padding, return the text as is
    return text

def des_decrypt(hex_ciphertext, key):
    # Ensure the key is exactly 8 characters
    if len(key) != 8:
    ...
    for i in range(0, len(ciphertext), 8):
        ...
        # 16 rounds - use keys in reverse order for decryption
...
if __name__ == "__main__":
    ...
    try:
        # Remove any spaces from the hex input
